...
//---------添加想买记录
    ...
    {
        $sql = "SELECT `idea_like`.* ,`user_info`.`user_name` from `idea_like`,`user_info` where `user_info`.`user_id`=`idea_like`.`liker_id` and `idea_like`.`idea_id`=" . $idea_id." and `idea_like`.`like_type`=0";
        $res = $this->db->get_results($sql, ARRAY_A);
        return $res;
    }
}
